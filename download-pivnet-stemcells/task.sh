...
# ensure the stemcell version found in the manifest exists on pivnet
  if [[ $(pivnet-cli pfs -p stemcells -r "$stemcell_version") == *"release not found"* ]]; then
    if [[ $(pivnet-cli pfs -p stemcells-ubuntu-xenial -r "$stemcell_version") == *"release not found"* ]]; then
      abort "Could not find the required stemcell version ${stemcell_version}. This version might not be published on PivNet yet, try again later."
    fi
  fi
...
#  done
  ...
  if [[ $(pivnet-cli pfs -p stemcells-ubuntu-xenial -r "$stemcell_version") == *"release not found"* ]]; then
    ...
    for product_file_id in $(pivnet-cli pfs -p stemcells-ubuntu-xenial -r "$stemcell_version" --format json | jq .[].id); do
      local product_file_name
      product_file_name=$(pivnet-cli product-file -p stemcells-ubuntu-xenial -r "$stemcell_version" -i "$product_file_id" --format=json | jq .name)
      if echo "$product_file_name" | grep -iq "$IAAS_TYPE"; then
        pivnet-cli download-product-files -p stemcells-ubuntu-xenial -r "$stemcell_version" -i "$product_file_id" -d "$download_dir" --accept-eula
        return 0
      fi
    done
  fi
